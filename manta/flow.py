...
def partial_diffusion(graph, iterations, limit, subset, ratio, permutations, seed=11111, verbose=False):
    """
    ...
    :param seed: Random seed, if 11111 no seed is used
    :param verbose: Verbosity level of function
    ...
    while b < subnum and true_iterations < (10 * subnum):
        ...
        balanced = harary_components(subgraph, seed, verbose=False)
...
def harary_components(graph, seed=11111, verbose=False):
    """
...
def harary_balance(graph, seed=11111):
    """
    ...
    :param graph: NetworkX graph
    :param seed: Random seed, 11111 means no seed is used
    :return: Returns a dictionary with connected components as keys;
             components that are balanced have a True value.
    """
    if seed != 11111:
